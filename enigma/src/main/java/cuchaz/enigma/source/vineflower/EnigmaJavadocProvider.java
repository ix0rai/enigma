...
public class EnigmaJavadocProvider implements IFabricJavadocProvider {
	...
	public String getClassDoc(StructClass structClass) {
		if (this.remapper != null) {
			EntryMapping mapping = getClassEntry(structClass).getMapping();
			...
			if (isRecord(structClass)) {
				for (StructRecordComponent component : structClass.getRecordComponents()) {
					EntryMapping componentMapping = getFieldEntry(structClass, component).getMapping();
	...
	public String getFieldDoc(StructClass structClass, StructField structField) {
		...
		if (this.remapper != null && !component) {
			EntryMapping mapping = getFieldEntry(structClass, structField).getMapping();
			...
			if (children != null && !children.isEmpty()) {
				for (Entry<?> child : children) {
					if (child instanceof LocalVariableEntry) {
						EntryMapping paramMapping = child.getMapping();
