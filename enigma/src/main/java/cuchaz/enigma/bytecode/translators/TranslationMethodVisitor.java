...
public class TranslationMethodVisitor extends MethodVisitor {
	...
	public TranslationMethodVisitor(Translator translator, EntryIndex index, ClassEntry ownerEntry, MethodEntry methodEntry, int api, MethodVisitor mv) {
		super(api, mv);
	...
	public void visitFieldInsn(int opcode, String owner, String name, String desc) {
		FieldEntry entry = new FieldEntry(index.getClass(owner), name, new TypeDescriptor(desc));
	...
	public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
		MethodEntry entry = new MethodEntry(index.getClass(owner), name, new MethodDescriptor(desc));
	...
	private Object[] getTranslatedFrame(Object[] array, int count) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (object instanceof String type) {
				array[i] = this.translator.translate(index.getClass(type)).getFullName();
			}
		}
	...
	public AnnotationVisitor visitTypeAnnotation(int typeRef, TypePath typePath, String desc, boolean visible) {
		...
		return new TranslationAnnotationVisitor(this.translator, this.index, typeDesc.getTypeEntry(this.index), this.api, av);
	}

	@Override
	public void visitTypeInsn(int opcode, String type) {
		ClassEntry translatedEntry = this.translator.translate(index.getClass(type));
	...
	public void visitInvokeDynamicInsn(String name, String desc, Handle bsm, Object... bsmArgs) {
		...
		for (int i = 0; i < bsmArgs.length; i++) {
			translatedBsmArgs[i] = AsmObjectTranslator.translateValue(this.index, bsmArgs[i]);
		}

		super.visitInvokeDynamicInsn(name, translatedMethodDesc.toString(), AsmObjectTranslator.translateHandle(this.index, bsm), translatedBsmArgs);
	}

	@Override
	public void visitLdcInsn(Object cst) {
		super.visitLdcInsn(AsmObjectTranslator.translateValue(this.index, cst));
	}
